...
static const char* EventErrno2Str(int rc)
{
    return zerror(rc);
}
...
CClusterManager::CClusterManager() :
        m_runState(EN_State::UnKnown), m_nodeMode(EN_NodeMode::Other), m_tag("xunce tech adc"), m_timeout(10)
{
...
int CClusterManager::Initialize(const std::string &hosts, const std::string &path, const std::string &node, unsigned int timeout)
{
    int res = 0;
    std::lock_guard<std::mutex> lock_guard(m_initMutex);
    do {
        if (m_runState == EN_State::Initialized || m_runState == EN_State::Running || m_runState == EN_State::Started) {
        ...
        zhandle_t *zk_handle = zookeeper_init(hosts.c_str(), CClusterManager::watcher, timeout, (clientid_t*) nullptr, (void*) this, 0);
        if (zk_handle == nullptr) {
            res = -2;
            cout << "Initialize breaked,error_info=" << EventErrno2Str(res) << endl;
            break;
        }
...
void CClusterManager::watcher(zhandle_t *zk_handle, int eventType, int state, const char *path, void *context)
{
    cout << "**********watcher event***********************" << "event:" << eventType << " state:" << state << " path:" << path << endl;

    std::cout << "watcher() thread_id is "<< get_thread_id() << std::endl;
...
{
    if (state == ZOO_EXPIRED_SESSION_STATE || state == ZOO_AUTH_FAILED_STATE || state == 999) {
...
void CClusterManager::onZOO_CHILD_EVENT(zhandle_t *zk_handle, int eventType, int state, const char *path)
{
...
int CClusterManager::createNode(const std::string &path, const std::string &node)
{
    ...
    if ((m_zkhandlePtr.get() != nullptr) && (path.length() > 0) && (node.length() > 0)) {
        ...
        std::string zkPath;
        if (res == ZNONODE) {
            std::vector<std::string> nodes = StrUtils::Split(path, "/", false);
            for (size_t i = 0; i < nodes.size(); ++i) {
                zkPath.append("/");
                zkPath.append(nodes[i]);
                res = zoo_exists(m_zkhandlePtr.get(), zkPath.c_str(), 0, nullptr);
                if (res == ZNONODE) {
                    res = zoo_create(m_zkhandlePtr.get(), zkPath.c_str(), m_tag.c_str(), m_tag.length(), &ZOO_OPEN_ACL_UNSAFE, 0, nullptr, 0);
                    cout << "create:" << zkPath << ":" << EventErrno2Str(res) << endl;
                    if (res != ZOK) {
                        res = -2;
                        break;
                    }
        ...
        res = zoo_exists(m_zkhandlePtr.get(), zkPath.c_str(), 0, nullptr);
        if (res == ZNONODE) {
            res = zoo_create(m_zkhandlePtr.get(), zkPath.c_str(), m_tag.c_str(), m_tag.length(), &ZOO_OPEN_ACL_UNSAFE, ZOO_EPHEMERAL, nullptr, 0);
            cout << "create:" << zkPath << ":" << EventErrno2Str(res) << endl;
        }
    } else {
        res = -1;
