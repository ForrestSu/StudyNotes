...
#include <set>
#include "ex_china.hpp"

namespace QuantLib {

    ExChina::ExChina(Market m) {
        // all calendar instances share the same implementation instance
        static ext::shared_ptr<Calendar::Impl> sseImpl(new ExChina::SseImpl);
        static ext::shared_ptr<Calendar::Impl> szseImpl(new ExChina::SzseImpl);
        static ext::shared_ptr<Calendar::Impl> IBImpl(new ExChina::IbImpl);
    ...
    bool ExChina::SseImpl::isWeekend(Weekday w) const {
        return w == Saturday || w == Sunday;
    }

    bool ExChina::SseImpl::isBusinessDay(const Date& date) const {
    ...
    bool ExChina::IbImpl::isWeekend(Weekday w) const {
        return w == Saturday || w == Sunday;
    }

    bool ExChina::IbImpl::isBusinessDay(const Date& date) const {
    ...
    /// SzseImpl
    bool ExChina::SzseImpl::isWeekend(Weekday w) const {
        return w == Saturday || w == Sunday;
    }

    bool ExChina::SzseImpl::isBusinessDay(const Date& date) const {
        // If it is already a SSE business day, it must be a Szse business day
        return sseImpl->isBusinessDay(date);
    }
}
