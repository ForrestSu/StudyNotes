...
public:
    ...
    void yield () {
        ...
        if (zk_io_poll_hdl && uv_is_active((uv_handle_t*) zk_io_poll_hdl)) {
            uv_poll_stop(zk_io_poll_hdl);
        }
        ...
        if (fd == -1 ) {
            if (zk_io_poll_hdl) {
                uv_close((uv_handle_t*) zk_io_poll_hdl, delete_on_close);
                zk_io_poll_hdl = NULL;
            }
            return;
        }
        ...
        if (oldFd != fd) {
            ...
            zk_io_poll_hdl = (uv_poll_t*)malloc(sizeof(uv_poll_t));
            if (!zk_io_poll_hdl) {
                LOG_ERROR(("Failed to malloc memory for uv_poll_t"));
                return;
             }

             zk_io_poll_hdl->data = this;
             #ifdef WIN32
             	uv_poll_init_socket(uv_default_loop(), zk_io_poll_hdl, fd);
             #else
             	uv_poll_init(uv_default_loop(), zk_io_poll_hdl, fd);
             #endif
        }

        LOG_DEBUG(("yield: starting poll for %lp from thread %lp", this));
        uv_poll_start(zk_io_poll_hdl, events, &zk_io_cb);
...
#define AW_METHOD_PROLOG(nargs) \
    ...
    void realClose (int code) {
        ...
        if (zhandle) {
            ...
            if (zk_io_poll_hdl) {
                int rc = uv_poll_stop(zk_io_poll_hdl);
