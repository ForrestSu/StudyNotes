...
/* parse program arguments */
	...
	if (enable_http) {
		if (!pgm_http_init (PGM_HTTP_DEFAULT_SERVER_PORT, &pgm_err)) {
			g_error ("Unable to start HTTP interface: %s\n", pgm_err->message);
			pgm_error_free (pgm_err);
			pgm_shutdown();
			pgm_messages_shutdown();
			return EXIT_FAILURE;
	...
	if (enable_snmpx) {
		if (!pgm_snmp_init (&pgm_err)) {
			g_error ("Unable to start SNMP interface: %s\n", pgm_err->message);
			pgm_error_free (pgm_err);
#ifdef CONFIG_WITH_HTTP
			if (enable_http)
				pgm_http_shutdown ();
...
{
	g_message ("on_console_ctrl (dwCtrlType:%lu)\n", (unsigned long)dwCtrlType);
...
/* parse network parameter into transport address structure */
	if (!pgm_getaddrinfo (g_network, NULL, &res, &pgm_err)) {
		g_error ("parsing network parameter: %s\n", pgm_err->message);
		goto err_abort;
	}
...
/* set IP parameters */
	...
	if (!pgm_connect (g_sock, &pgm_err)) {
		g_error ("connecting PGM socket: %s\n", pgm_err->message);
		goto err_abort;
	}
...
/* create receiver thread */
	...
	g_thread = g_thread_create_full (receiver_thread,
					 ...
					 &glib_err);
	if (!g_thread) {
		g_error ("g_thread_create_full failed errno %i: \"%s\"\n", glib_err->code, glib_err->message);
	...
	)
{
	...
	if (efd < 0) {
		g_error ("epoll_create failed errno %i: \"%s\"\n", errno, strerror(errno));
		g_main_loop_quit (g_loop);
		return NULL;
	}
	...
	{
		g_error ("pgm_epoll_ctl failed errno %i: \"%s\"\n", errno, strerror(errno));
		g_main_loop_quit (g_loop);
		return NULL;
	}
	...
	{
		g_error ("epoll_ctl failed errno %i: \"%s\"\n", errno, strerror(errno));
		g_main_loop_quit (g_loop);
		return NULL;
	}
  ...
  enum {
	...
	PGM_IO_STATUS_NORMAL = 1,		/* success */
	PGM_IO_STATUS_RESET = 2,		/* session reset */
	PGM_IO_STATUS_FIN = 3,		/* session finished */
	PGM_IO_STATUS_EOF = 4,		/* socket closed */
	PGM_IO_STATUS_WOULD_BLOCK = 5,	/* resource temporarily unavailable */
	PGM_IO_STATUS_RATE_LIMITED =6,	/* would-block on rate limit, check timer */
	PGM_IO_STATUS_TIMER_PENDING =7,	/* would-block with pending timer */
...
{
	    // printf("succ recv %ld bytes data!\n",len);
        guint i = 0;
/* for each apdu */
	do {
...
/* truncate to first fragment to make GLib printing happy */
		...
		if (msgv[i].msgv_len > 1)
			...
			//FIXME: will execute here
            //printf("-->\t%u: \"%s\" (%" G_GSIZE_FORMAT " bytes from %s)\n",
			//	   i, buf, apdu_len, tsi);
		}
		if(iCounter%1000 == 0){
			printf("%ld data = <%s>\n", iCounter , buf);
