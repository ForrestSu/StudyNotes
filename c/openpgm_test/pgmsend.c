...
/* parse program arguments */
	...
	while ((c = getopt (argc, argv, "s:n:p:r:f:K:N:lih")) != -1)
	{
		...
		case 'r':	{g_max_rte = atoi (optarg);printf("max rate = %d\n",g_max_rte);} break;

		case 'f':	{g_fec = TRUE; puts("Enable FEC.");}break;
	...
	if (g_fec && ( !g_k || !g_n )) {
		pgm_messages_shutdown();
		g_error ("Invalid Reed-Solomon parameters RS(%d, %d).\n", g_n, g_k);
		usage (binary_name);
	}
...
/* setup signal handlers */
	...
	{
		puts("create_pgm_socket OK!");
		printf("optind = %d, argc = %d\n ",optind, argc);
		int nCounter = 100000;
		int k = 0;
		const char* str_data ="sunquan hello world!\0";
		const int data_len = strlen(str_data)+1;
		while (k <= nCounter) {
			const int status = pgm_send (g_sock, str_data, data_len, NULL);
		        if (PGM_IO_STATUS_NORMAL != status) {
				  puts("pgm_send failed.\n");
		        }
		    if(k % 1000 == 0) {
				printf("%d send %d bytes ok\n", k, data_len);
			}
			k++;
			//sleep(1);
		}
	}
	puts("send complete,will sleep 10s!");
	getchar();
	puts("clean up!");
...
create_pgm_socket (void)
{
    ...
    printf("g_udp_port = %d, create_pgm_socket...\n", g_udp_encap_port);
/* parse network parameter into PGM socket address structure */
	...
	if (g_udp_encap_port) {
		if (!pgm_socket (&g_sock, sa_family, SOCK_SEQPACKET, IPPROTO_UDP, &pgm_err)) {
			g_error ("Creating PGM/UDP socket: %s\n", pgm_err->message);
			goto err_abort;
		}
		...
		if (!pgm_socket (&g_sock, sa_family, SOCK_SEQPACKET, IPPROTO_PGM, &pgm_err)) {
			g_error ("Creating PGM/IP socket: %s\n", pgm_err->message);
			goto err_abort;
		}
	}
...
/* create global session identifier */
	...
	printf("g_port %d, default = %d\n", g_port, DEFAULT_DATA_DESTINATION_PORT);
